...
(defn ^:private shell-output-cache-dir
  ...
  (let [cache-home (or (System/getenv "XDG_CACHE_HOME")
                       (io/file (config/get-property "user.home") ".cache"))
        ws-hash (workspaces-hash workspaces)]
    (io/file cache-home "eca" ws-hash "shell-output")))
        ...
        (let [work-dir (or (some-> user-work-dir fs/canonicalize str)
          ...
          (cond
            ...
            (let [output (or out "")
                  ...
                  threshold (get-in config [:toolCall :shellCommand :outputFileThreshold] 2000)]
